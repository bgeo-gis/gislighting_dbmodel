...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
		...
		*/
        ...
        -- FEATURE INSERT
				INSERT INTO dma (id, idval, descript,  the_geom, undelete,  expl_id)
				VALUES (NEW.id, NEW.idval, NEW.descript, NEW.the_geom, NEW.undelete, expl_id_int);
    ...
    ELSIF TG_OP = 'UPDATE' THEN
   	-- FEATURE UPDATE
			UPDATE dma 
			SET id=NEW.id, idval=NEW.idval, descript=NEW.descript, the_geom=NEW.the_geom, undelete=NEW.undelete, expl_id=NEW.expl_id
			WHERE id=NEW.id;
	 ...
	 -- FEATURE DELETE
		DELETE FROM dma WHERE id = OLD.id;		
